...
newExplainer.add("controller", module => {
  ...
  return angularModule => {
    ...
    if( !hasRegistered("controller", _name)){
      invokeQueue["controller"] = invokeQueue["controller"] || [];
      invokeQueue["controller"].push( _name );
      ...
      console.error(`controller, named ${_name} is already exist, so will not be registered`)
    }
  }
});
newExplainer.add("service", module => {
  ...
  return angularModule => {
    let _name = camelhill(name);
    if( !hasRegistered(type || "factory", _name)){
      invokeQueue[type || "factory"] = invokeQueue[type || "factory"] || [];
      invokeQueue[type || "factory"].push( _name );
      angularModule[type || "factory"](_name, params);
    } else {
      console.error(`${type || "factory"}, named ${_name} is already exist, so will not be registered`)
    }
  }
});
